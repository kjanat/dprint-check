...
const parseCheckAnnotations = (output, listDifferent) => {
	...
	let current;
	const finish = () => {
		if (current !== void 0) annotations.push(current);
		current = void 0;
	};
	for (const line of lines) {
		...
		else if (current !== void 0 && current.line === void 0) {
			const lineNumber = originalLine(line);
			if (lineNumber !== void 0) current.line = lineNumber;
		}
	}
	finish();
	return annotations;
...
const checkFormatting = async (binaryPath, configPath, additionalArgs, options = {}) => {
	const { annotations = true, annotate = error, execute = execFileAsync } = options;
...
const restorePluginCache = async (cacheDir, version, platformKey, binaryPath, config, configRoots) => {
...
const run = async () => {
	...
	try {
		...
		if (cacheAvailable) await restorePluginCache(cacheDir, version, platformKey, location, config, preparedConfig?.roots ?? []);
